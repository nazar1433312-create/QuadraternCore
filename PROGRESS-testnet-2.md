...
- **LWMA-модуль реализован, но НЕ проходит все тесты** (2 из 8 всё ещё падают — см. выше) и **не подключён** к `pow.cpp`/`GetNextWorkRequired`. Подключать к цепи рано — сначала починить клэмп-логику.
...
1. **Разобраться, почему клэмп-тесты (`consistently_slow_...`, `consistently_fast_...`) всё ещё падают** после фикса переполнения — приоритет №1, LWMA нельзя подключать к цепи, пока это не зелёное. Начать с печати промежуточных значений (`nextTarget` до и после клэмпа, `maxUp`/`maxDown`, `lastTarget`) в самом тесте — сейчас неясно, баг в клэмп-сравнении или в основной формуле при solve-time на границе антиманипуляционного ограничения (6×target).
2. Подключить `qtrn::LwmaNextTarget` к `pow.cpp` (`GetNextWorkRequired`/`CalculateNextWorkRequired`) — только после п.1.
...
7. При работе с `arith_uint256` (или любым fixed-width big-int) — прикидывать битовую длину промежуточных произведений перед тем, как писать `a * b / c`; при риске переполнения делить сначала на как можно больший множитель знаменателя, только потом умножать. Проверить, нет ли похожего риска переполнения/потери точности ещё и в самой клэмп-логике (`lastTarget * PERCENT / 100`).
